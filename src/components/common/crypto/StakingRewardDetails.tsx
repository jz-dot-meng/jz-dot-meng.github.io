...
//styling
import styles from "./StakingRewardDetails.module.css";
...
export const StakingRewardDetails: React.FunctionComponent<
  StakingRewardDetailsType
> = ({ ...props }) => {
  const { blockchain, address, rewards } = props;

  const balances = useSelector(
    (state: ReduxRootState) => state.crypto.addressTracker.balances
  );
  ...
  useEffect(() => {
    ...
    const addr = balances.token[blockchain].filter(
      (addresses: { [address: string]: TokenBalancePayload }) =>
        addresses.hasOwnProperty(address)
    )[0];
    // get ticker
    const ticker = getRewardTickerFromBlockchain(blockchain);
    setTicker(ticker);
    // get wallet balance
    const tokenBalances = addr[address]
      .filter((tokenTicker) => tokenTicker.hasOwnProperty(ticker))
      .map((data) => Number(data[ticker]));
    ...
    const now = Date.now();
    const daysSinceFirstReward = Math.floor(
      (now - firstRewardDate) / (1000 * 60 * 60 * 24)
    );
    ...
    let last = calculateApyFromRewardAndTotal(
      rewards[rewards.length - 1]["reward_amount"],
      totalBalance,
      roughPeriod
    );
    setLastApy(last);
  ...
  return (
    <>
      <div className={styles.rewardDetailsContainer}>
        <div className={styles.rewardDetailHeading}>Wallet balance</div>
        <div
          className={styles.rewardDetailData}
        >{`${walletBalance} ${ticker}`}</div>

        <div className={styles.rewardDetailHeading}>
          Total rewards accumulated to date
        </div>
        <div className={styles.rewardDetailData}>{`${rewardSum.toFixed(
          6
        )} ${ticker}`}</div>

        <div className={styles.rewardDetailHeading}>
          Average daily reward (since first reward)
        </div>
        <div className={styles.rewardDetailData}>{`${averageReward.toFixed(
          6
        )} ${ticker}`}</div>
        ...
        <div className={styles.rewardDetailData}>{`${lastApy.toFixed(
          4
        )}%`}</div>

        <div className={styles.rewardDetailHeading}>
          APY based on average reward
        </div>
        <div className={styles.rewardDetailData}>{`${averageApy.toFixed(
          4
        )}%`}</div>
      </div>
    </>
