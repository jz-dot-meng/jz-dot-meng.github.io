...
export const MemoryTiles: React.FunctionComponent<MemoryTilesType> = ({ ...props }) => {
    const { state, display, grid, levelSelection, handleNextLevel, handleGameOver, preventClick } = props;
    ...
    const removeAllSelectedTileClass = (array: number[]) => {
        array.forEach(number => {
            document.getElementById(`tile-${number}`)?.classList.remove('memory-correctSelection')
        })
    }
                ...
                <div style={{
                    ...
                    {tileMap.map(number => (
                        <button
                            ...
                            className={
                                `memory-gametile 
                                ${display === 'Memorise' ? (levelSelection.indexOf(number) !== -1) ? 'memory-correctSelection' : '' : ''} 
                                ${display === 'GameOver' ? (levelSelection.indexOf(number) !== -1 && selectionSplicer.indexOf(number) === -1) ? 'memory-missingSelection' : '' : ''}
                                `
                            }
                            onClick={(e) => validateTile(number, e)} disabled={display !== 'Guess'}></button>
                    ))}
                </div>
            </div>
        </>
