...
export const Memory: React.FunctionComponent<MemoryType> = ({ ...props }) => {
    ...
    const generateArray = (level: number) => {
        ...
        const totalLevelTiles = memoryLevels[level].dim[0] * memoryLevels[level].dim[1];
        ...
        <>
            ...
            <div className='memory-body'>
                ...
                <GameTemplate state={gameState} config={memoryConfig}>
                    <MemoryTiles state={gameState} display={displayMode} grid={memoryLevels[level].dim} levelSelection={levelSelectionIndices} handleNextLevel={handleNextLevel} handleGameOver={handleGameOver} preventClick={() => setDisplayMode('Memorise')} />
                </GameTemplate>
            </div>
