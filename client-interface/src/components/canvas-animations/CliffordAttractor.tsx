...
export const CliffordAttractor: React.FunctionComponent = ({ ...props }) => {
    ...
    const constantChangeRate = [
        Math.random() / (Math.random() * 200),
        Math.random() / (Math.random() * 200),
        Math.random() / (Math.random() * 200),
        Math.random() / (Math.random() * 200),
    ];
    const colourChangeRate = [
        (Math.random() - 0.5) / 30,
        (Math.random() - 0.5) / 30,
        (Math.random() - 0.5) / 30,
    ];
    ...
    function calcColour(index: number) {
        return (
            ...
            128 *
                Math.sin(
                    colourChangeRate[index] * t.current + (colourInit[index] * 2 * Math.PI) / 255
                )
        );
    }
    ...
    function calcConstant(index: number) {
        return (
            ...
            1.5 *
                Math.sin(
                    constantChangeRate[index] * t.current + (constantInit[index] * 2 * Math.PI) / 6
                )
        );
    }
    ...
    function draw(x: number, y: number, ctx: CanvasRenderingContext2D) {
        ctx.fillStyle =
            "rgba(" +
            Math.ceil(red.current) +
            "," +
            Math.ceil(green.current) +
            "," +
            Math.ceil(blue.current) +
            ",1)"; //`rgba(${colourArr[1]},${colourArr[2]},${colourArr[3]},0.7)`
        ctx.beginPath();
    ...
    return <canvas style={{ cursor: "pointer" }} ref={canvasRef} onClick={reinit}></canvas>;
};
